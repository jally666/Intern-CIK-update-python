...
in_file = pd.read_excel(main_location,sheetname="Wrong IssuerName")
...
####################################################
#This part is added for some problematic issuernames
####################################################
issuer=[issuer[i].replace(" AND "," ") for i in range(len(issuer))]
issuer=[issuer[i].replace(" LP","") for i in range(len(issuer))]
issuer=[issuer[i].replace("COM "," COM ") for i in range(len(issuer))]
issuer=[issuer[i].replace("NET "," NET ") for i in range(len(issuer))]
issuer=[re.sub(r'^A ', '', issuer[i]) for i in range(len(issuer))]
issuer=[re.sub(r'^THE ', '', issuer[i]) for i in range(len(issuer))]
#####################################################
	...
	if len(i) >= 2:  
		...
		while True: 
			...
			if no_issuer==u'No matching companies.':
				...
				# scenerio 1: the keyword directly relate to one specific company, this part is useful for company="", eg, "huawei"
				...
				if pd.isnull(matches):
					try:
						...
						pass
				# scenerio 2 : use "contain", find all ciks that contain this key word
				else:
					try:
...
print "\n %d issuers is problematic again and need mannually check" %(len(wrong_issuer)) 		
writer = ExcelWriter(results)
results_df.to_excel(writer,'Possible Companies',index=False)
wrong_issuer.to_excel(writer,'Wrong IssuerName mannual needed',index=False)
writer.save()
